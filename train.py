...
from torch.cuda.amp import autocast, GradScaler  # Добавлено для AMP [[9]]
...
def train_epoch(epoch, train_loader, criterion, optimizer, batch_size, scheduler):
    ...
    for i, sample_batch in enumerate(train_loader):
        ...
        imgs = imgs.to(device)
        true_masks = true_masks.long().to(device)  # Преобразование в LongTensor [[4]]

        with autocast():  # Включение автоматического смешанного обучения [[1]]
            outputs = net(imgs)
        ...
        if i % 100 == 0:
            ...
            writer.add_figure('predictions vs. actuals', plot_net_predictions(imgs, true_masks, masks_pred, batch_size), global_step=i + len(train_loader) * epoch)

        optimizer.zero_grad()
        # Использование GradScaler для масштабирования градиентов [[9]]
...
def train_net(train_loader, val_loader, net, device, epochs=5, batch_size=1, lr=0.1, save_cp=True):
    ...
    best_precision = 0
    scaler = GradScaler()  # Инициализация GradScaler [[9]]
